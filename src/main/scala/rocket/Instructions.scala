...
object Instructions {
  ...
  /* SAVVINA: added for MetaSys but not existing in MetaSys repo, added by me 
  def AMU_CLEAR_ACR     = BitPat("b00000110000000000111000001011011")
  def AMU_MAP           = BitPat("b0000110??????????111000001011011")
  def AMU_MAP_2D        = BitPat("b0001111??????????111000001011011")
  def AMU_READ_ACR      = BitPat("b00000010000000000111?????1011011")
  def AMU_READ_LEN      = BitPat("b00001010000000000111?????1011011")
  def AMU_READ_ROWCNT   = BitPat("b00011100000000000111?????1011011")
  def AMU_READ_STRIDE   = BitPat("b00011000000000000111?????1011011")
  def AMU_UNMAP         = BitPat("b0000111??????????111000001011011")
  def AMU_UNMAP_2D      = BitPat("b0010000??????????111000001011011")
  def AMU_WRITE_ACR     = BitPat("b0000010?????00000111000001011011")
  def AMU_WRITE_LEN     = BitPat("b0000100?????00000111000001011011")
  def AMU_WRITE_ROWCNT  = BitPat("b0001101?????00000111000001011011")
  def AMU_WRITE_STRIDE  = BitPat("b0001011?????00000111000001011011")
  def AST_ACTIVATE      = BitPat("b0001000?????00000111000001011011")
  def AST_DEACTIVATE    = BitPat("b0001001?????00000111000001011011")
  def AST_STATUS        = BitPat("b0001010?????00000111?????1011011")
  def ATOM_ACTIVATE     = BitPat("b????????????????????000011111011")
  def ATOM_DEACTIVATE   = BitPat("b????????????????????000101111011")
  def ATOM_M_INIT       = BitPat("b000011100000?????111?????1110111")
  def ATOM_MAP          = BitPat("b0000100??????????111?????1110111")
  def ATOM_MSTATUS      = BitPat("b000011000000?????111?????1110111")
  def ATOM_STATUS       = BitPat("b000000100000?????111?????1110111")
  def ATOM_UNMAP        = BitPat("b000010100000?????111?????1110111")
  def FATOM             = BitPat("b0010001??????????111000001011011")
  def FATOM_LOAD        = BitPat("b0010011??????????111000001011011")
  def FATOM_SELECT      = BitPat("b0010010??????????111000001011011")
  def BC_ATOM_SEL       = BitPat("b0010100??????????111000001011011")
  def LD_ACR            = BitPat("b000001000000?????111?????1110111")
  def ST_ACR            = BitPat("b00000110000000000111?????1110111")
*/
