...
class AMULookUpResp2048X1 extends Bundle{
  val valid = Output(Bool())
  val miss = Output(Bool())
  val atom_id = Output(UInt(11.W))
  val xcpt = Output(Bool())
}

class AMULookUpIO2048X1 extends Bundle{
  val req  = new AMULookUpReq
  val resp = new AMULookUpResp2048X1
}
...
class AtomLookUpUnit(val atomGranularity: Int = 9, albSize: Int = 32, isPhysical: Boolean = false)(implicit p: Parameters) extends Module{
  ...
  // io.mem            <> DontCare // SAVVINA commented out: Why is that necessary?
  ...
  io.lookup.resp.xcpt         := (state === s_writealb && xcpt) || io.acrbase.orR === 0.U
...
class BuffedALU(val atomGranularity: Int = 9, albSize: Int = 32, isPhysical: Boolean = false)
  ...
  val io = IO(new Bundle{
    val lookup  = new AMULookUpIO
    val mem     = new HellaCacheIO
    val acrbase = Input(UInt(39.W))
    val phybase = Input(UInt(39.W))
  ...
  s0_valid              := io.lookup.req.valid && !(io.acrbase.orR === 0.U)
  ...
  albreq.wen       := s4_valid && (s4_atom_id =/= 0.U) // SAVVINA added: Do not write 0 atom ids to ALB
  ...
  io.lookup.resp.xcpt     := (s3_valid && xcpt) || (io.acrbase.orR === 0.U)
...
/*-----------Differences from BuffedALU-------------*/
/* Writes in the ALB happen only if atom_id is not equal to zero */

class BuffedALUInitial(val atomGranularity: Int = 9, albSize: Int = 32, isPhysical: Boolean = false)
      (implicit p: Parameters) extends Module{
  val io = IO(new Bundle{
    val lookup  = new AMULookUpIO
    val mem     = new HellaCacheIO
...
class BuffedALU2048X14rthTRYChangeSizeInitial(val atomGranularity: Int = 9, albSize: Int = 32, isPhysical: Boolean = false)
      (implicit p: Parameters) extends Module{
  val io = IO(new Bundle{
    val lookup  = new AMULookUpIO2048X1
    val mem     = new HellaCacheIO
    val acrbase = Input(UInt(39.W))
    val phybase = Input(UInt(39.W))
    val map_update_addr   = Input(UInt(40.W))
  ...
  val alb     = Module(new Buffed_ALB_FA2048X1(atomGranularity,ovralbsize))
  //val alb     = Module(new Buffed_ALB_RANDOM(atomGranularity,ovralbsize))
  val albreq  = alb.io.req
  val albresp = alb.io.resp
  alb.io.map_update_id := io.map_update_id
  ...
  val s3_base_lookup = s3_addr >> atomGranularity.U
  val s1_base_lookup = s1_addr >> atomGranularity.U

  val half_lookup = (1.U << (32 - atomGranularity)).asUInt

  val s3_offset_lookup = Mux(s3_base_lookup(0) === 1.U, s3_base_lookup + 1.U, s3_base_lookup + half_lookup)

  val s1_offset_lookup = Mux(s1_base_lookup(0) === 1.U, s1_base_lookup + 1.U, s1_base_lookup + half_lookup)

  val s3_fixed_addr     = Mux(isPhysical.B, (s3_offset_lookup + io.acrbase + io.phybase), 
                                              (s3_offset_lookup + io.acrbase))
  val s1_fixed_addr     = Mux(isPhysical.B, (s1_offset_lookup + io.acrbase + io.phybase), 
                                              (s1_offset_lookup + io.acrbase))
  io.mem.req.bits.addr  := Mux(s3_retry, s3_fixed_addr, s1_fixed_addr)
  s1_advance            := io.mem.req.fire() && !s3_retry
  // ~~~~~~~~~~~~~~~~~~~S2~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  // also s1 in D$'s POV, nothing to do here really
  ...
  s3_valid              := Mux(s2_valid, true.B, s3_valid && !(resp_valid || xcpt || nackd))
  s3_addr               := Mux(s2_valid, s2_addr, s3_addr)  
  val s3_atom_id        = io.mem.resp.bits.data(10,0)
  block_queue           := ((s3_valid || s3_retry) && ((s3_addr >> atomGranularity.U) === (s0_addr >> atomGranularity.U))) ||
                           (s1_valid && ((s1_addr >> atomGranularity.U) === (s0_addr >> atomGranularity.U))) ||
                           (s2_valid && ((s2_addr >> atomGranularity.U) === (s0_addr >> atomGranularity.U))) 
  //inv_s1                := ((s3_addr >> atomGranularity.U) === (s1_addr >> atomGranularity.U)) && 
  ...
  io.mem.req.bits.tag  := 0.U
  io.mem.req.bits.cmd  := M_XRD // read only should be enough
  io.mem.req.bits.size := log2Ceil(2).U
  io.mem.req.bits.signed := false.B
  io.mem.req.bits.phys := isPhysical.B
  io.mem.req.bits.dprv := PRV.U.U
  io.mem.req.bits.dv := false.B
...
class BuffedALU2048X14rthTRYChangeSize(val atomGranularity: Int = 9, albSize: Int = 32, isPhysical: Boolean = false)
      (implicit p: Parameters) extends Module{
  val io = IO(new Bundle{
    val lookup  = new AMULookUpIO2048X1
    val mem     = new HellaCacheIO
  ...
  albreq.wen       := s4_valid && (s4_atom_id =/= 0.U)
  albreq.atom_id   := s4_atom_id
  albreq.paddr_wr  := s4_addr
...
/*--------Existing in original MetaSys--------*/
/*--------Differences from BuffedALUDualALB ----------*/
/* Introduces a second ALB module and selects the right ALB module to read from/write to */
/* A weekness is that it doesn't take care for the case albSize is zero, like the BuffedALU module does */
...
/*----------Differences from BuffedALU----------*/
/*----------Fixed lookup.resp.atom_id logic----------*/
class BuffedALUDualALBSavvinaChanges(val atomGranularity: Int = 9, albSize1: Int = 64, albSize2: Int = 64, isPhysical: Boolean = false)
      (implicit p: Parameters) extends Module{
  val io = IO(new Bundle{
    val lookup  = new AMUDualALBLookUpIO
    val mem     = new HellaCacheIO
  ...
  albreq2.rden           := (s0_valid && (s1_advance || !s1_valid)) && !block_queue &&/*!inv_s0 &&*/ 
                                   !io.map_update_valid && !io.lookup.req.moduleid        
  //s0_miss               := Mux((albSize == 0).B, s0_valid && io.lookup.req.ready, Mux(io.lookup.req.moduleid, albresp1.miss, albresp2.miss)) // SAVVINA changed
  s0_miss               := Mux(io.lookup.req.moduleid, albresp1.miss, albresp2.miss)
  io.lookup.req.ready   := (s1_advance || !s1_valid) && !block_queue && !io.map_update_valid
  // ~~~~~~~~~~~~~~~~~~~S1~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  // Lookup in AMT which is in main memory, also s0 in D$'s POV when io.mem.req.fire()
  ...
  val albrespatom_id_reg = RegInit(0.U(8.W)) // SAVVINA added

  when(s4_mid)
  {
    albreq1.wen       := s4_valid && (s4_atom_id =/= 0.U) // SAVVINA changed
    albrespatom_id_reg := albresp1.atom_id // SAVVINA added
  }.otherwise
  {
    albreq2.wen       := s4_valid && (s4_atom_id =/= 0.U) // SAVVINA changed
    albrespatom_id_reg := albresp2.atom_id // SAVVINA added
  }

  io.lookup.resp.valid    := RegNext(s0_valid && io.lookup.req.ready && !s0_miss) || s4_valid
  io.lookup.resp.miss     := DontCare
  io.lookup.resp.atom_id  := Mux(s4_valid, s4_atom_id, RegNext(albrespatom_id_reg)) // SAVVINA changed
  io.lookup.resp.xcpt     := (s3_valid && xcpt) || (io.acrbase.orR === 0.U)
...
class AMULookUpRespReturnModuleId extends Bundle{
  val valid = Output(Bool())
  val miss = Output(Bool())
  val atom_id = Output(UInt(8.W))
  val xcpt = Output(Bool())
  val moduleid = Output(Bool())
}

class AMUDualALBLookUpReturnModuleIdIO extends Bundle{
  val req  = new AMUDualALBLookUpReq
  val resp = new AMULookUpRespReturnModuleId
}

/*-----------Differences from BuffedALUDualALBSavvinaChanges-------------*/
/* Added moduleid as a signal in the resp interface, so that the module also returns the moduleid to which the resp signals belong */
class BuffedALUDualALBSavvinaChangesReturnModuleId(val atomGranularity: Int = 9, albSize1: Int = 64, albSize2: Int = 64, isPhysical: Boolean = false)
      (implicit p: Parameters) extends Module{
  val io = IO(new Bundle{
    val lookup  = new AMUDualALBLookUpReturnModuleIdIO // SAVVINA changed
    val mem     = new HellaCacheIO
    val acrbase = Input(UInt(39.W)) 
    val phybase = Input(UInt(39.W))
    val map_update_addr   = Input(UInt(40.W))
  ...
  val s2_valid    = RegInit(false.B)
  val s3_valid    = RegInit(false.B)
  val s0_mid      = RegInit(false.B) // SAVVINA added
  val s1_mid      = RegInit(false.B)
  val s2_mid      = RegInit(false.B)
  val s3_mid      = RegInit(false.B)
  val s0_miss     = WireInit(false.B)
  ...
  /* SAVVINA added START */
  val s0_resp_fire = s0_valid && io.lookup.req.ready && !s0_miss 
   
  s0_mid                := Mux(s0_resp_fire, io.lookup.req.moduleid, s0_mid) // SAVVINA added

  when (s0_resp_fire) {
    albrespatom_id_reg := Mux(io.lookup.req.moduleid, albresp1.atom_id, albresp2.atom_id)
  }
  /* SAVVINA added END */
  ...
  io.lookup.resp.valid    := RegNext(s0_valid && io.lookup.req.ready && !s0_miss) || s4_valid
  io.lookup.resp.miss     := DontCare
  io.lookup.resp.atom_id  := Mux(s4_valid, s4_atom_id, albrespatom_id_reg) // SAVVINA changed
  io.lookup.resp.xcpt     := (s3_valid && xcpt) || (io.acrbase.orR === 0.U)
  io.lookup.resp.moduleid := Mux(s4_valid, s4_mid, s0_mid) // SAVVINA added
