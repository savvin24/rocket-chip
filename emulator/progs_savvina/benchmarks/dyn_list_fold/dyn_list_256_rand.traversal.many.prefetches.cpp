...
int main() {
    ...
    void ** attributes = (void **) malloc(NPREF * sizeof(void*));

    uint64_t **arr = (uint64_t **) malloc(256 * sizeof(uint64_t*));
    ...
    for (int i=0; i<3000; i++)
    {
        ...
        if (tagID < 256)
        {
            if ((i % NPREF) == 0) 
            {
                atom_map((void *) current_ptr, sizeof(Node)/(1<<GRANULARITY)+1, ++tagID); 
                if (tagID > 0) 
                {
                    attributes[NPREF-1] = (void *) current_ptr;
                    atom_define_bulk(tagID-1, attributes, NPREF);
                }        
            }     
            else 
            {
                attributes[(i % NPREF)-1] = (void *) current_ptr;
            }       
        }
        #endif
    ...
    for (int i=0; i<4000; i++)
    {
        ...
        nthElement = getNthElement(randIndex);
        #ifdef DEBUG
    ...
    printf ("cycles_alloc: %lu\n", cycles_alloc2);
    printf ("cycles_traversal: %lu\n", cycles_traversal2);
