...
int main() {
    ...
    #ifndef NOATOM 
        atom_map((void*) head, sizeof(Node)/(1<<GRANULARITY)+1, 2040);
    #endif
    ...
    insert(data);
        ...
        atom_define(2040, 0, (void*) current_ptr); // Define the atom with the current pointer
    #endif
    ...
    Node* n0 = getNthElement(1); // Read accesses to 1st and 2nd node
    Node* n1 = getNthElement(0); // Read access to 1st node, must have a hit in 1st level mapping MetaSys cache
