...
int main() {
    ...
    for (int i=0; i<2048; i++)
    {
        insert(i);
        #ifndef NOATOM
        if (i < 2047)  atom_map((void *) current_ptr, sizeof(Node)/(1<<GRANULARITY)+1, i); // Map the current pointer to atom 5
    ...
    {
        int randIndex = rand() % 2048; // Random index between 0 and 19999
