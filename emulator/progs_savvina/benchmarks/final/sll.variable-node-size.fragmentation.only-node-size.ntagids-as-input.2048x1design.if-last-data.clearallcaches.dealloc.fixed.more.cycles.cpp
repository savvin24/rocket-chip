...
void deleteList() {
    ...
    while (current != nullptr) {
        #ifndef NOATOM
            if((count++) < ntagIDs) atom_unmap((void *) current, mapSize); // Unmap the current pointer from atom 0
        #endif
        next = current->next;
        free(current);
        current = next;
...
int main(int argc, char** argv) {
    ...
    unsigned long cycles_atom_init_start, cycles_atom_init_end, instret_atom_init_start, instret_atom_init_end;

    asm volatile ("rdinstret %0" : "=r" (instret_atom_init_start));
    asm volatile("rdcycle %0" : "=r" (cycles_atom_init_start));

    // Allocate tagID table in main memory (even at baseline)
    #ifndef NOATOM
        uint16_t* MMTptr = atom_init_16((int)GRANULARITY, false);
    #else
        uint16_t* MMTptr = atom_init_16((int)GRANULARITY, true);
        // asm volatile("" :: "r"(MMTptr) : "memory");
    #endif

    asm volatile("rdcycle %0" : "=r" (cycles_atom_init_end));
    asm volatile ("rdinstret %0" : "=r" (instret_atom_init_end));
    ...
    unsigned long instret_alloc_start, instret_alloc_end, cycles_alloc_start, cycles_alloc_end;

    asm volatile ("rdinstret %0" : "=r" (instret_alloc_start));
    ...
    asm volatile("rdinstret %0" : "=r" (instret_alloc_end));

    volatile Node *nthElement;

    unsigned long instret_traversal_1, instret_traversal_2, cycles_traversal1, cycles_traversal2;

    asm volatile("rdinstret %0" : "=r" (instret_traversal_1));
    ...
    asm volatile("rdinstret %0" : "=r" (instret_traversal_2));
    ...
    unsigned long instret_dealloc_1, instret_dealloc_2, cycles_dealloc_start, cycles_dealloc_end;
    unsigned long instret_dealloc_flush_pmt_start, cycles_dealloc_mmt_start, cycles_dealloc_mmt_end;
    unsigned long cycles_dealloc_flush_pmt_start, cycles_dealloc_flush_pmt_end, instret_dealloc_flush_pmt_end;

    asm volatile("rdinstret %0" : "=r" (instret_dealloc_1));
    asm volatile("rdcycle %0" :"=r" (cycles_dealloc_start));
    
    deleteList();

    asm volatile("rdcycle %0" :"=r" (cycles_dealloc_end));
    asm volatile("rdinstret %0" : "=r" (instret_dealloc_2));
    asm volatile("rdcycle %0" :"=r" (cycles_dealloc_mmt_start));    

    free(MMTptr);

    asm volatile("rdcycle %0" :"=r" (cycles_dealloc_mmt_end));
    asm volatile("rdinstret %0" : "=r" (instret_dealloc_flush_pmt_start));
    asm volatile("rdcycle %0" :"=r" (cycles_dealloc_flush_pmt_start));

    #ifndef NOATOM
        uint64_t *flush_arr = (uint64_t *) malloc(sizeof(uint64_t));
        *(flush_arr) = 0x0;

        // for(int i=1; i < ntagIDs; i++)
        for(int i=0; i < 2048; i++)
        {
            atom_define(i, 0, (void *)flush_arr); 
        }
    #endif

    asm volatile("rdcycle %0" :"=r" (cycles_dealloc_flush_pmt_end));
    asm volatile("rdinstret %0" : "=r" (instret_dealloc_flush_pmt_end));

    printf ("Cycles for MMT allocation: %lu\n", cycles_atom_init_end - cycles_alloc_start);
    ...
    printf ("Cycles for list deallocation: %lu\n", cycles_dealloc_end - cycles_dealloc_start);
    printf ("Cycles for MMT deallocation: %lu\n", cycles_dealloc_mmt_end - cycles_dealloc_mmt_start);
    printf ("Cycles for PMT flush: %lu\n", cycles_dealloc_flush_pmt_end - cycles_dealloc_flush_pmt_start);

    printf ("Instructions for MMT allocation: %lu\n", instret_atom_init_end - instret_alloc_start - 2);
    printf ("Instructions for allocation: %lu\n", instret_alloc_end - instret_alloc_start - 2);
    printf ("Instructions for traversal: %lu\n", instret_traversal_2 - instret_traversal_1 - 2);
    printf ("Instructions for list deallocation: %lu\n", instret_dealloc_2 - instret_dealloc_1 - 2);
    printf ("Instructions for MMT deallocation: %lu\n", instret_dealloc_flush_pmt_start - instret_dealloc_2 - 2);
    printf ("Instructions for PMT flush: %lu\n", instret_dealloc_flush_pmt_end - instret_dealloc_flush_pmt_start - 2);
