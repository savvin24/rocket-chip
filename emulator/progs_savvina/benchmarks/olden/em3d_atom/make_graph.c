...
void fill_table(node_t **node_table, double *values, int size, int procname)
{
  ...
  int i, tagID_offset = 0;
  
  prev_node = (node_t *)malloc(sizeof(node_t));
  #ifndef NOATOM
      if (count_fill_table == 2) tagID_offset = ntagIDs/2 - 1;
      atom_map((void *) prev_node, mapSize, tagID_offset + 1);
  #endif
  node_table[0] = prev_node;
  *values = gen_uniform_double();
  prev_node->value = values++;
  prev_node->from_count = 0; // SAVVINA COMMENT: from_count tracks the number of incoming edges to this node
  
  /* Now we fill the node_table with allocated nodes */
  for (i=1; i<size; i++) {
    cur_node = (node_t *)malloc(sizeof(node_t));
    #ifndef NOATOM
      if (i < (ntagIDs/2 - 1)) atom_map((void *) cur_node, mapSize, tagID_offset+i+1);
      #ifdef DEBUG
        chatting("iteration n.%d, involved node vaddr:%p, ntagIDs: %d, mapSize: %" PRIu32 ", count_fill_table: %d,  assigned tagID: %d\n", i, cur_node, ntagIDs, mapSize, count_fill_table, tagID_offset+i+1);
      #endif
      #endif
    *values = gen_uniform_double();
    ...
    #ifndef NOATOM
      if ((i > npref) && (i < ntagIDs/2 + npref)) atom_define(tagID_offset+i-npref, 0, (void *) cur_node); 
      #ifdef DEBUG
        chatting("iteration n.%d, npref: %d, tagID: %d\n", i, npref, tagID_offset+i-npref);
      #endif
    #endif
    prev_node = cur_node;
  }
...
#if 0
        if ((((unsigned double double) other_node) >> 7) < 2048)
...
void update_from_coeffs(node_t *nodelist) {
  ...
  for (cur_node = nodelist; cur_node; cur_node=cur_node->next) {
    ...
    if (from_count < 1) {
      ...
      cur_node->from_length = 0; // SAVVINA COMMENT: from_length tracks the current number of entries in from_values and coeffs arrays. By the end of execution it should equal from_count.
    } else {
