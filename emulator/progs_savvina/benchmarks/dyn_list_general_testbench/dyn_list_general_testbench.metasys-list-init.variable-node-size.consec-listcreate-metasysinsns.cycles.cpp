...
uint32_t mapSize;
uint16_t tagID = 1; // Global tagID variable
...
void insert_after(struct Node *insertionpt, uint64_t data) {
    ...
    #ifndef NOATOM
        if (tagID < 256)
            atom_map((void *) newNode, mapSize, tagID); // Map the new node to the next atom
        atom_define_lookup((void *) insertionpt, 0, (void *) newNode); // Define the atom with the insertion point and new node
    #endif
    ...
    if(next!=NULL && tagID < 256)
    {
        atom_define(tagID++, 0, (void *) next); 
    }
    else if(next==NULL)
    {
        tagID++;
...
int main(int argc, char *argv[]) { // Modified main function signature
    ...
    if (argc < 5) {
        cerr << "Usage: " << argv[0] << " <NODESIZE> <nnodes_value> <metasys_list_init_file.txt> <rand_index_file.txt>" << endl;
        return 1; // Indicate an error
    }

    NODESIZE = atoi(argv[1]); // given in bytes
    int nnodes = atoi(argv[2]);
    const char* metasys_list_init_filename = argv[3];
    ...
    #ifdef DEBUG
        printf("nnodes: %d, ninitindices: %d, ntravers: %d\n", nnodes, ninitindices, ntravers);
    #endif
    ...
    #ifndef NOATOM
        atom_map((void *) head, mapSize, tagID++); // Map the head node to atom 1
    #endif
