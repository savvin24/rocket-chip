...
void init_queue(int size){ 
    ...
    queue.pointer_end = 0;
    printf("In init_queue: Returning from init_queue\n");
}

void push_back(int element){
    queue.value[queue.pointer_end] = element;
    queue.pointer_end = queue.pointer_end + 1 ;
    printf("IN PUSH_BACK: Returning from push_back\n");
}
...
void rad(){
        ...
        int round =0;
        int w=-1;
        //for(int times=0; times<50; times++){ //savvina comment
...
int main(int argc, char* argv[]){
        ...
        radii_init();
        rad();
        printf("IN MAIN: Returned from rad\n");
