...
struct csr{   //SAVVINA comment: CSR (Compressed Sparse Row) format for the graph
...
void bfs(){
    ...
    for(int x = 0 ; x < 10 ; x++)
    {
        ...
        { 
            ...
            //Savvina: Added for debug reasons
            for(int i=0; i<queue.pointer_end; i++)
            {
                printf("value no.%d in queue is %d\n", i, queue.value[i]);
            }
        } 
        free(queue.value);
    }
